...
func SelectedfetchPosts(w http.ResponseWriter, r *http.Request) {
	...
	url := fmt.Sprintf("http://backend:8000/api/v1/getpost/%s", postID) // backend = DOCKER'da çalışırıyor.  Localde çalışması için http://localhost:8000/api/v1/getpost/%s", postID)  değişikliği gerek
